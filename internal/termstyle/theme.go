...
func ResolveTheme(opts ThemeOptions) (Theme, error) {
	...
	if opts.NoColor || envTruthy(env["DANGIT_NO_COLOR"]) || env["NO_COLOR"] != "" {
		theme.NoColor = true
	}
...
func resolveThemeFile(file string, env map[string]string, skipDefault bool) (string, bool) {
	...
	if value := strings.TrimSpace(env["DANGIT_THEME_FILE"]); value != "" {
	...
	return filepath.Join(configDir, "dangit", "theme.conf"), false
}
