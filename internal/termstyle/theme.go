...
func ResolveTheme(opts ThemeOptions) (Theme, error) {
	file, explicitFile := termtheme.ResolveThemeFile("dangit", opts.File, opts.Env, opts.SkipDefaultFile)
	...
	if termtheme.EnvNoColor("dangit", opts.Env, opts.NoColor) {
		theme.NoColor = true
	}
...
func ThemeConfigPath(file string, env []string) (string, error) {
	path, _ := termtheme.ResolveThemeFile("dangit", file, env, false)
	if path == "" {
